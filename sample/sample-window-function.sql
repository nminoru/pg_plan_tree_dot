...
--- SELECT city, ID, salary, rank() OVER (PARTITION BY city ORDER BY salary DESC) FROM employee;

SELECT generate_plan_tree_dot('SELECT city, ID, salary, rank() OVER (PARTITION BY city ORDER BY salary DESC) FROM employee;', 'sample-window-function.dot');
