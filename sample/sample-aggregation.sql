...
--- SELECT city, sum(ID), avg(salary), count(*) FROM employee GROUP BY city HAVING avg(salary) > 5000 ORDER BY city;
SELECT generate_plan_tree_dot('SELECT city, sum(ID), avg(salary), count(*) FROM employee GROUP BY city HAVING avg(salary) > 5000 ORDER BY city;', 'sample-aggregation.dot');
