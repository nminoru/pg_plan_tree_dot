...
DROP FUNCTION F EXISTS concat_selected_fields(in_t employee);
...
CREATE FUNCTION concat_selected_fields(in_t employee) RETURNS text AS $$
BEGIN
    RETURN in_t.ID || in_t.name || in_t.city || in_t.region;
END;
$$ LANGUAGE plpgsql;

-- SELECT concat_selected_fields(employee.*) FROM employee;
SELECT generate_plan_tree_dot('SELECT concat_selected_fields(employee.*) FROM employee;', 'sample-whole-row.dot');

DROP FUNCTION concat_selected_fields(in_t employee);
DROP TABLE employee;
