...
CREATE INDEX idx ON employee (name, city, region);
SET enable_seqscan = off;

--- SELECT * FROM employee WHERE ID > 2 AND (name, city, region) IN (('Robert', 'New York', 'W'), ('Linda', 'New York', 'N'), ('Mary', 'Toronto', 'W'));
SELECT generate_plan_tree_dot('SELECT * FROM employee WHERE ID > 2 AND (name, city, region) IN ((''Robert'', ''New York'', ''W''), (''Linda'', ''New York'', ''N''), (''Mary'', ''Toronto'', ''W''));', 'sample-bitmapor.dot');
