...
--- SELECT city, sum(ID), avg(salary), count(*) FROM employee GROUP BY CUBE(region, city);
SELECT generate_plan_tree_dot('SELECT city, sum(ID), avg(salary), count(*) FROM employee GROUP BY CUBE(region, city);', 'sample-aggregation-cube.dot');
