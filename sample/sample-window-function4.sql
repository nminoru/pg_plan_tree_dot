...
--- SELECT city, ID, salary, rank() OVER w, avg(salary) OVER w FROM employee WINDOW w AS (PARTITION BY city ORDER BY salary DESC);

SELECT generate_plan_tree_dot('SELECT city, ID, salary, rank() OVER w, avg(salary) OVER w FROM employee WINDOW w AS (PARTITION
 BY city ORDER BY salary DESC);', 'sample-window-function4.dot');
