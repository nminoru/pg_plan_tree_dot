...
--- SELECT city, sum(ID), avg(salary), count(*) FROM employee GROUP BY ROLLUP(region, city) ORDER BY region, city;
SELECT generate_plan_tree_dot('SELECT city, sum(ID), avg(salary), count(*) FROM employee GROUP BY ROLLUP(region, city) ORDER BY region, city;', 'sample-aggregation-rollup.dot');
